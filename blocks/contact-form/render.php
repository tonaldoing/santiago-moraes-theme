<?php
/**
 * Render: sm/contact-form
 *
 * Contact form block — simple (inline) or full (stacked).
 * Both variants use the same sm_contact_form AJAX handler.
 *
 * @package Santiago_Moraes
 *
 * @var array    $attributes Block attributes.
 * @var string   $content    InnerBlocks content (unused).
 * @var WP_Block $block      Block instance.
 */

defined( 'ABSPATH' ) || exit;

$form_type   = $attributes['formType'] ?? 'full';
$show_social = $attributes['showSocial'] ?? false;

// Social links for the optional social row.
$social_data = array(
	'sm_social_instagram' => array(
		'label' => 'Instagram',
		'svg'   => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path d="M224.1 141c-63.6 0-114.9 51.3-114.9 114.9s51.3 114.9 114.9 114.9S339 319.5 339 255.9 287.7 141 224.1 141zm0 189.6c-41.1 0-74.7-33.5-74.7-74.7s33.5-74.7 74.7-74.7 74.7 33.5 74.7 74.7-33.6 74.7-74.7 74.7zm146.4-194.3c0 14.9-12 26.8-26.8 26.8-14.9 0-26.8-12-26.8-26.8s12-26.8 26.8-26.8 26.8 12 26.8 26.8zm76.1 27.2c-1.7-35.9-9.9-67.7-36.2-93.9-26.2-26.2-58-34.4-93.9-36.2-37-2.1-147.9-2.1-184.9 0-35.8 1.7-67.6 9.9-93.9 36.1s-34.4 58-36.2 93.9c-2.1 37-2.1 147.9 0 184.9 1.7 35.9 9.9 67.7 36.2 93.9s58 34.4 93.9 36.2c37 2.1 147.9 2.1 184.9 0 35.9-1.7 67.7-9.9 93.9-36.2 26.2-26.2 34.4-58 36.2-93.9 2.1-37 2.1-147.8 0-184.8zM398.8 388c-7.8 19.6-22.9 34.7-42.6 42.6-29.5 11.7-99.5 9-132.1 9s-102.7 2.6-132.1-9c-19.6-7.8-34.7-22.9-42.6-42.6-11.7-29.5-9-99.5-9-132.1s-2.6-102.7 9-132.1c7.8-19.6 22.9-34.7 42.6-42.6 29.5-11.7 99.5-9 132.1-9s102.7-2.6 132.1 9c19.6 7.8 34.7 22.9 42.6 42.6 11.7 29.5 9 99.5 9 132.1s2.7 102.7-9 132.1z"/></svg>',
	),
	'sm_social_spotify'   => array(
		'label' => 'Spotify',
		'svg'   => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 496 512"><path d="M248 8C111.1 8 0 119.1 0 256s111.1 248 248 248 248-111.1 248-248S384.9 8 248 8zm100.7 364.9c-4.2 0-6.8-1.3-10.7-3.6-62.4-37.6-135-39.2-206.7-24.5-3.9 1-9 2.6-11.9 2.6-9.7 0-15.8-7.7-15.8-15.8 0-10.3 6.1-15.2 13.6-16.8 81.9-18.1 165.6-16.5 237 26.2 6.1 3.9 9.7 7.4 9.7 16.5s-7.1 15.4-15.2 15.4zm26.9-65.6c-5.2 0-8.7-2.3-12.3-4.2-62.5-37-155.7-51.9-238.6-29.4-4.8 1.3-7.4 2.6-11.9 2.6-10.7 0-19.4-8.7-19.4-19.4s5.2-17.8 15.5-20.7c27.8-7.8 56.2-13.6 97.8-13.6 64.9 0 127.6 16.1 177 45.5 8.1 4.8 11.3 11 11.3 19.7-.1 10.8-8.5 19.5-19.4 19.5zm31-76.2c-5.2 0-8.4-1.3-12.9-3.9-71.2-42.5-198.5-52.7-280.9-29.7-3.6 1-8.1 2.6-12.9 2.6-13.2 0-23.3-10.3-23.3-23.6 0-13.6 8.4-21.3 17.4-23.9 35.2-10.3 74.6-15.2 117.5-15.2 73 0 149.5 15.2 205.4 47.8 7.8 4.5 12.9 10.7 12.9 22.6 0 13.6-11 23.3-23.2 23.3z"/></svg>',
	),
	'sm_social_youtube'   => array(
		'label' => 'YouTube',
		'svg'   => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512"><path d="M549.655 124.083c-6.281-23.65-24.787-42.276-48.284-48.597C458.781 64 288 64 288 64S117.22 64 74.629 75.486c-23.497 6.322-42.003 24.947-48.284 48.597-11.412 42.867-11.412 132.305-11.412 132.305s0 89.438 11.412 132.305c6.281 23.65 24.787 41.5 48.284 47.821C117.22 448 288 448 288 448s170.78 0 213.371-11.486c23.497-6.321 42.003-24.171 48.284-47.821 11.412-42.867 11.412-132.305 11.412-132.305s0-89.438-11.412-132.305zm-317.51 213.508V175.185l142.739 81.205-142.739 81.201z"/></svg>',
	),
	'sm_social_bandcamp'  => array(
		'label' => 'Bandcamp',
		'svg'   => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M256 8C119 8 8 119 8 256S119 504 256 504 504 393 504 256 393 8 256 8zm48.2 326.1h-181L207.9 178h181z"/></svg>',
	),
	'sm_social_facebook'  => array(
		'label' => 'Facebook',
		'svg'   => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M512 256C512 114.6 397.4 0 256 0S0 114.6 0 256c0 120 82.7 220.8 194.2 248.5V334.2h-56.6v-78.2h56.6v-61.3c0-56.4 33.5-87.3 84.6-87.3 24.5 0 50.2 4.4 50.2 4.4v55.4h-28.3c-27.8 0-36.5 17.3-36.5 35.1v42.2h62.4l-10 78.2h-52.4v170.3C429.3 476.8 512 376 512 256z"/></svg>',
	),
	'sm_social_twitter'   => array(
		'label' => 'Twitter/X',
		'svg'   => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M389.2 48h70.6L305.6 224.2 487 464H345L233.7 318.6 106.5 464H35.8L200.7 275.5 26.8 48H172.4L272.9 180.9 389.2 48zM364.4 421.8h39.1L151.1 88h-42L364.4 421.8z"/></svg>',
	),
);

// Unique form IDs for the block (avoid collision with multiple forms on a page).
$form_uid = 'sm-block-contact-' . wp_unique_id();

if ( 'simple' === $form_type ) :
	// --- Simple / inline form (same as homepage contact). ---
	?>
	<div class="home-contact__form-wrap">
		<form class="home-contact__form" id="<?php echo esc_attr( $form_uid ); ?>" method="post">
			<input type="hidden" name="action" value="sm_contact_form">
			<?php wp_nonce_field( 'sm_contact_form', 'sm_contact_nonce' ); ?>

			<div class="contact-form__hp" aria-hidden="true">
				<label for="<?php echo esc_attr( $form_uid ); ?>-hp">Website</label>
				<input type="text" name="sm_website" id="<?php echo esc_attr( $form_uid ); ?>-hp" tabindex="-1" autocomplete="off">
			</div>

			<input type="hidden" name="sm_name" value="Visitante">

			<div class="home-contact__fields">
				<label for="<?php echo esc_attr( $form_uid ); ?>-email" class="sr-only"><?php esc_html_e( 'Tu email', 'santiago-moraes' ); ?></label>
				<input
					type="email"
					name="sm_email"
					id="<?php echo esc_attr( $form_uid ); ?>-email"
					class="home-contact__input"
					placeholder="<?php esc_attr_e( 'Tu email', 'santiago-moraes' ); ?>"
					required
				>
				<label for="<?php echo esc_attr( $form_uid ); ?>-msg" class="sr-only"><?php esc_html_e( 'Tu mensaje', 'santiago-moraes' ); ?></label>
				<input
					type="text"
					name="sm_message"
					id="<?php echo esc_attr( $form_uid ); ?>-msg"
					class="home-contact__input home-contact__input--message"
					placeholder="<?php esc_attr_e( 'Tu mensaje', 'santiago-moraes' ); ?>"
					required
				>
				<button type="submit" class="btn btn--primary btn--sm">
					<?php esc_html_e( 'Enviar', 'santiago-moraes' ); ?>
				</button>
			</div>

			<div class="home-contact__status" role="alert"></div>
		</form>
	</div>
	<?php
else :
	// --- Full / stacked form (same as contact page). ---
	?>
	<div class="contact-form-section">
		<div class="contact-form-section__inner">

			<form class="contact-form" id="<?php echo esc_attr( $form_uid ); ?>" method="post" novalidate>
				<?php wp_nonce_field( 'sm_contact_form', 'sm_contact_nonce' ); ?>
				<input type="hidden" name="action" value="sm_contact_form">

				<div class="contact-form__hp" aria-hidden="true">
					<label for="<?php echo esc_attr( $form_uid ); ?>-hp">Website</label>
					<input type="text" name="sm_website" id="<?php echo esc_attr( $form_uid ); ?>-hp" tabindex="-1" autocomplete="off">
				</div>

				<div class="contact-form__field">
					<label for="<?php echo esc_attr( $form_uid ); ?>-name" class="contact-form__label"><?php esc_html_e( 'Nombre', 'santiago-moraes' ); ?> *</label>
					<input type="text" name="sm_name" id="<?php echo esc_attr( $form_uid ); ?>-name" class="contact-form__input" required>
				</div>

				<div class="contact-form__field">
					<label for="<?php echo esc_attr( $form_uid ); ?>-email" class="contact-form__label"><?php esc_html_e( 'Email', 'santiago-moraes' ); ?> *</label>
					<input type="email" name="sm_email" id="<?php echo esc_attr( $form_uid ); ?>-email" class="contact-form__input" required>
				</div>

				<div class="contact-form__field">
					<label for="<?php echo esc_attr( $form_uid ); ?>-subject" class="contact-form__label"><?php esc_html_e( 'Asunto', 'santiago-moraes' ); ?></label>
					<input type="text" name="sm_subject" id="<?php echo esc_attr( $form_uid ); ?>-subject" class="contact-form__input">
				</div>

				<div class="contact-form__field">
					<label for="<?php echo esc_attr( $form_uid ); ?>-message" class="contact-form__label"><?php esc_html_e( 'Mensaje', 'santiago-moraes' ); ?> *</label>
					<textarea name="sm_message" id="<?php echo esc_attr( $form_uid ); ?>-message" class="contact-form__textarea" rows="6" required></textarea>
				</div>

				<div class="contact-form__status" role="alert"></div>

				<button type="submit" class="btn btn--primary contact-form__submit">
					<?php esc_html_e( 'Enviar Mensaje', 'santiago-moraes' ); ?>
				</button>
			</form>

		</div>
	</div>
	<?php
endif;

// Optional social links row.
if ( $show_social ) :
	$active_social = array();
	foreach ( $social_data as $mod_key => $info ) {
		$url = sm_get_option( $mod_key, '' );
		if ( $url ) {
			$active_social[ $mod_key ] = array_merge( $info, array( 'url' => $url ) );
		}
	}

	if ( $active_social ) :
		?>
		<div class="home-contact__social">
			<?php foreach ( $active_social as $data ) : ?>
				<a href="<?php echo esc_url( $data['url'] ); ?>" class="home-contact__social-link" target="_blank" rel="noopener noreferrer" aria-label="<?php echo esc_attr( $data['label'] ); ?>">
					<?php echo $data['svg']; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Static SVG. ?>
				</a>
			<?php endforeach; ?>
		</div>
		<?php
	endif;
endif;
